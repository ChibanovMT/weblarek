...
- Методы:
  ...
  - `validate(): { payment?: string; email?: string; phone?: string; address?: string }` — выполняет проверку обязательных полей на непустые значения и возвращает объект ошибок. Свойство присутствует только в случае ошибки соответствующего поля.
